...
- [Task: Metal-Accelerated Terminal Surface View & 120 FPS ProMotion Sync](file:///Users/rafaelkscharf/Projects/homelab/SwarmDeck/wayfinder/tickets/task-metal-terminal-surface-resolution.md) ([#40](https://github.com/RafaelScharf/SwarmDeck/issues/40)) — Implemented `TerminalSurfaceView` via `NSViewRepresentable` wrapping `libghostty-spm` Metal pipeline with `CAMetalLayer` triple-buffering (`maximumDrawableCount = 3`) and VSync sync (`displaySyncEnabled = true`) for 120 FPS ProMotion, Darwin `ioctl(TIOCSWINSZ)` deduplication and safe clamping, `suppressesPixelOnlyResizes` preventing sub-cell redraw storms, dynamic font scaling (`Cmd+`/`Cmd-`/`Cmd0`), curated Ghostty themes, and clean clipboard pasteboard integration with `NSPasteboard.general`.
...
- [x] [Task: Metal-Accelerated Terminal Surface View & 120 FPS ProMotion Sync](https://github.com/RafaelScharf/SwarmDeck/issues/40) *(Blocked by #37, #38)*
