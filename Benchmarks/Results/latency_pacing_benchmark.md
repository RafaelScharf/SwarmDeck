...
- **Timestamp:** `2026-09-08T20:16:35Z`
- **Status:** **PASSED**
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Idle Latency (0 Background Sessions)** | 300 | **0.05 ms** | 0.17 ms | **0.28 ms** | 0.07 ms | 0.02 ms | 0.74 ms | 0.06 ms | $\le 12\text{ ms}$ | $\le 25\text{ ms}$ | **PASS (Target)** |
| **Loaded Latency (4 Background Streams)** | 300 | **0.14 ms** | 0.22 ms | **0.34 ms** | 0.15 ms | 0.05 ms | 0.68 ms | 0.06 ms | $\le 15\text{ ms}$ | $\le 25\text{ ms}$ | **PASS (Target)** |
...
| **Total Frames Analyzed** | Full duration sample set | **595 frames** (4.96s) | Validated |
| **Dropped Frames (> 10.42 ms)** | 0 frames (Hard limit $\le 2\text{/s}$) | **0 frames** (0.00/s) | **ZERO DROPS (PASS)** |
| **Frame Delta Jitter ($\sigma$)** | $< 0.50\text{ ms}$ (Hard limit $\le 1.50\text{ ms}$) | **0.007 ms** | **EXCELLENT (< 0.5ms)** |
| **Frame Delta Range** | Min .. Max interval | 8.30 ms .. 8.36 ms | Mean: 8.33 ms |
...
| **SwarmDeck Measured (This Run)** | **0.14 ms** | **0.34 ms** | **120.0 FPS** | **0.007 ms** | **0 drops** |
