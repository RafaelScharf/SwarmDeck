...
// MARK: - OutputStateDetector Prototype
...
print("Starting Agent State Detection Prototype...\n")

// Scenario 1: Aider booting up, doing some work, showing a prompt
print("\n--- Scenario 1: Aider Simulation ---")
var detector = OutputStateDetector()
let aiderChunks = [
    "Initializing Aider v1.0.0\n",
    "Loading model...\n",
    "Ready.\n",
    "aid",
    "er",
...
for chunk in aiderChunks {
    await detector.feed(data: chunk.data(using: .utf8)!)
    try? await Task.sleep(nanoseconds: 50_000_000)
}
try? await Task.sleep(for: .milliseconds(300))

// Scenario 2: Aider spinning (\r overwrites)
print("\n--- Scenario 2: \\r Overwrites (Spinners) ---")
detector = OutputStateDetector()
await detector.feed(data: "User input: write a script\n".data(using: .utf8)!)
try? await Task.sleep(for: .milliseconds(300))
...
for chunk in spinnerChunks {
    await detector.feed(data: chunk.data(using: .utf8)!)
    try? await Task.sleep(nanoseconds: 50_000_000)
}
try? await Task.sleep(for: .milliseconds(300))

// Scenario 3: Blocked / y/n prompt
print("\n--- Scenario 3: Blocked Prompt ---")
detector = OutputStateDetector()
...
// Scenario 4: OSC 133 Shell integration
print("\n--- Scenario 4: OSC 133 Shell Integration ---")
detector = OutputStateDetector()
await detector.feed(data: "\n\u{1b}]133;A\u{7}user@host $ ".data(using: .utf8)!)
try? await Task.sleep(nanoseconds: 50_000_000)
await detector.feed(data: "\u{1b}]133;B\u{7}".data(using: .utf8)!)
try? await Task.sleep(for: .milliseconds(300))

// Print final status
let finalState = await detector.currentState
print("\nFinal simulation state: \(finalState)")
