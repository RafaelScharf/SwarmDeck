...
/// Actor encapsulating pseudo-terminal (PTY) master I/O, window resizing, and process supervision.
public actor PTYService {
    public nonisolated let masterFD: Int32
    public nonisolated let childPID: pid_t
    ...
    public init(configuration: PTYConfiguration) throws {
        ...
        self.masterFD = m
        self.childPID = pid
        self.supervisor = ProcessSupervisor(pid: pid)
    }
    
    public init(preset: AgentPreset = .standardShell) throws {
    ...
    public func setOnData(_ handler: @escaping @Sendable (Data) -> Void) {
        self.onData = handler
    }
    
    public func setOnExit(_ handler: @escaping @Sendable (Int32) -> Void) {
        self.onExit = handler
    }
    
    @discardableResult
    public func terminate() async -> Int32 {
    ...
    public nonisolated func writeToMaster(_ data: Data) {
    ...
    public nonisolated func resize(columns: Int, rows: Int, widthPixels: Int, heightPixels: Int) {
        let closed = isClosed.withLock { $0 }
        guard !closed, masterFD >= 0 else { return }
    ...
    public nonisolated func getWindowSize() -> winsize? {
        let closed = isClosed.withLock { $0 }
